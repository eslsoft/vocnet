...
func (fe *FragmentEvaluator) Process(ctx context.Context, pctx *pipeline.PipelineContext) (*scoring.ProcessResult, error) {
	...
	allFragments := make(map[string][]*scoring.FieldFragment) // key: fieldKey, value: competing fragments
	...
	if pctx.EvaluatedFragments == nil {
		pctx.EvaluatedFragments = make(map[string][]*scoring.FieldFragment)
	}
...
func (fe *FragmentEvaluator) evaluateProvider(pctx *pipeline.PipelineContext, provider string) *scoring.EvaluatedFragments {
	...
	for _, lexeme := range pctx.Lexemes {
		...
		evaluated.Fragments[key] = &scoring.FieldFragment{
			Type:     "lexeme",
			Data:     lexeme,
			Score:    score,
			Provider: provider,
