...
type stubProcessor struct {
	name      string
	processFn func(ctx context.Context, pctx *PipelineContext) (*scoring.ProcessResult, error)
}

func (s *stubProcessor) Name() string { return s.name }

func (s *stubProcessor) Process(ctx context.Context, pctx *PipelineContext) (*scoring.ProcessResult, error) {
...
func TestPipelineRun_AbortOnStageError(t *testing.T) {
	...
	evaluator := scoring.NewDataEvaluator(scorer, testLogger())
...
func TestPipelineRun_ContinueOnExplicitProcessorSkip(t *testing.T) {
	...
	p := NewPipeline(
		...
		lemmaRepo,
		lexemeRepo,
		scoring.NewDataEvaluator(scoring.NewRuleBasedScorer(), testLogger()),
		testLogger(),
	)
