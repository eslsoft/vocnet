...
// Re-export from scoring for convenience
	...
	for _, rel := range incoming {
		...
		if entry, ok := index[key]; ok {
			...
			if newScore > entry.score ||
				(newScore == entry.score && scoring.SourceProviderTrustRank(rel.Provider) > scoring.SourceProviderTrustRank(existing[entry.idx].Provider)) {
