...
// --- Shared merge functions (package-level, used across evaluator, persistence, and enrichment) ---

// MergeSenses combines two sense lists, avoiding duplicates by language+gloss.
func MergeSenses(existing, new []entity.LexemeSense) []entity.LexemeSense {
...
func MergeFrequencies(existing, new []entity.Frequency) []entity.Frequency {
...
func MergePhonetics(existing, new []entity.Phonetic) []entity.Phonetic {
...
func MergeForms(existing, new []*entity.LemmaForm) []*entity.LemmaForm {
	...
	for _, newForm := range new {
		...
		if !exists {
			...
			merged := MergePhonetics(existingForm.Phonetics, newForm.Phonetics)
...
func EnrichLexeme(existing *entity.Lexeme, new *entity.Lexeme) *entity.Lexeme {
	enriched := *existing
	...
	// Merge senses
	if len(new.Senses) > 0 {
		enriched.Senses = MergeSenses(existing.Senses, new.Senses)
	}

	// Merge categories
	if len(new.Categories) > 0 {
		enriched.Categories = MergeCategories(existing.Categories, new.Categories)
	}
...
func (e *DataEvaluator) mergeLexemeFields(existing, new *entity.Lexeme) *entity.Lexeme {
	...
	if len(new.Senses) > 0 {
		merged.Senses = MergeSenses(existing.Senses, new.Senses)
	}

	// Categories: merge with deduplication
	if len(new.Categories) > 0 {
		merged.Categories = MergeCategories(existing.Categories, new.Categories)
	}
...
func (e *DataEvaluator) EvaluateAndMergeLemmaUpdate(
	...
	if len(update.Frequencies) > 0 {
		merged.Frequencies = MergeFrequencies(existing.Frequencies, update.Frequencies)
		decisions = append(decisions, AdoptionDecision{
			ShouldAdopt: true,
			NewSource:   newProvider,
...
// FormKey generates a unique key for a form based on surface and formType.
func FormKey(f *entity.LemmaForm) string {
	return fmt.Sprintf("%s:%s", f.Surface, f.FormType)
}
