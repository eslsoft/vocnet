...
run_test() {
    ...
    test_cmd="$test_cmd stdbuf -oL -eL go test -tags=quality -timeout=${timeout} ./internal/usecase/pipeline -run TestPipelineDataQualityGates"
...
case "$COMMAND" in
    ...
    all)
        ...
        go test -tags=quality -timeout=120m ./internal/usecase/pipeline \
