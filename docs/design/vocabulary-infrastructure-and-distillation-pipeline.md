...
### 5.1 Lemma：全球唯一锚点
*   **变更**：`Lemma` 实体增加 `WikidataQID` 字段。
*   **操作**：Phase 1 负责 QID 锁定。所有后续实体（Lexeme, Relation）必须通过 Lemma 关联到此 ID，实现全球数据互操作性。
...
### 6.1 Phase 1: Discovery (准入与标识锚定)
*   **职责**：验证并锚定全球唯一语义 ID (Wikidata QID)。
*   **具体任务**：
    *   **QID 检索**：利用词原型检索 Wikidata，锁定唯一的 QID，拒绝非标准词汇。
    *   **标准命名**：提取官方拼写规范（如 `iPhone` 非 `iphone`）。
    *   **歧义标记**：若一个词对应多个 QID，标记为 `Ambiguous` 待人工消歧。
